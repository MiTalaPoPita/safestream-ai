...
import { useFFmpegExport }               from "@/lib/useFFmpegExport";
    ...
    <div style={{ position:"fixed", inset:0, zIndex:600, background:"rgba(2,4,6,.9)", backdropFilter:"blur(20px)", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div className="glass" style={{ width:"100%", maxWidth:520, borderRadius:14, overflow:"hidden", boxShadow:"0 40px 100px rgba(0,0,0,.75)" }}>
        ...
        <div style={{ padding:"16px 22px", display:"flex", alignItems:"center", justifyContent:"space-between", borderBottom:"1px solid var(--border)" }}>
          <div>
            <p style={{ fontFamily:"var(--font-display, Syne, sans-serif)", fontSize:14, fontWeight:700, color:"var(--t1)" }}>Export Safe Version</p>
            <p style={{ fontFamily:"var(--font-mono, monospace)", fontSize:8.5, color:"var(--t3)", letterSpacing:".12em", marginTop:2 }}>FFMPEG.WASM · EDGE · NO UPLOAD</p>
          </div>
        ...
        <div style={{ padding:"16px 22px", borderBottom:"1px solid var(--border)" }}>
          <p style={{ fontFamily:"var(--font-mono, monospace)", fontSize:8, color:"var(--t3)", letterSpacing:".14em", marginBottom:10 }}>BAKED FILTER PARAMETERS</p>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:9 }}>
            {([
              ...
              <div key={label} style={{ background:"var(--s2)", border:"1px solid var(--border)", borderRadius:7, padding:"9px 11px" }}>
                <p style={{ fontFamily:"var(--font-mono, monospace)", fontSize:7.5, color:"var(--t3)", letterSpacing:".13em", marginBottom:4 }}>{label}</p>
        ...
        <div style={{ padding:"16px 22px", borderBottom:"1px solid var(--border)" }}>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
            <p style={{ fontFamily:"var(--font-mono, monospace)", fontSize:8.5, color:"var(--t3)", letterSpacing:".12em" }}>
              {label || PHASE_LABEL[phase] || (status==="idle"?"READY":"PROCESSING…")}
            </p>
            <p style={{ fontFamily:"var(--font-mono, monospace)", fontSize:11, fontWeight:700, color: status==="done"?"#10b981":status==="error"?"#f43f5e":"var(--t2)" }}>{totalPct}%</p>
          </div>
        ...
        <div style={{ padding:"16px 22px", display:"flex", flexDirection:"column", gap:9 }}>
          {status==="done" && result && (
            <div style={{ padding:"10px 13px", borderRadius:7, background:"rgba(16,185,129,.06)", border:"1px solid rgba(16,185,129,.18)", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <div>
                ...
                <p style={{ fontFamily:"var(--font-mono, monospace)", fontSize:8, color:"var(--t3)", letterSpacing:".1em" }}>{(result.sizeKB/1024).toFixed(1)} MB · H.264 · Digital Sunglasses baked</p>
              </div>
          ...
          <div style={{ display:"flex", gap:9 }}>
            {(status==="idle"||status==="error") && (
              <button className="btn-primary" onClick={() => run(source, params, outputName)} style={{ flex:1, padding:"10px 0", borderRadius:7, fontSize:9.5 }}>▶ Begin Export</button>
            )}
            {status==="done" && (
              <button className="btn-primary" onClick={reset} style={{ flex:1, padding:"10px 0", borderRadius:7, fontSize:9.5 }}>↺ Export Again</button>
            )}
            <button className="btn-ghost" onClick={handleShare} style={{ flex:1, padding:"10px 0", borderRadius:7, fontSize:9.5 }}>
              {copied ? "✓ Copied!" : "⎘ Safe-Share URL"}
            </button>
          </div>
          {shareURL && (
            <p style={{ fontFamily:"var(--font-mono, monospace)", fontSize:8, color:"var(--t3)", wordBreak:"break-all", lineHeight:1.6, padding:"7px 10px", borderRadius:6, background:"var(--s2)", border:"1px solid var(--border)" }}>{shareURL}</p>
          )}
        </div>
      </div>
    </div>
